...
export const CalendarsRepository = {
  ...
  async listByOwner(
    ownerId: string,
  ): Promise<(Calendar & { orders?: any[] })[]> {
    try {
      const { data, error } = await (supabase.from("calendars") as any)
        .select(
          "*, primary_color, secondary_color, background_url, header_message, footer_message, capsule_title, capsule_message, locked_title, locked_message, orders(id, status, expires_at)",
        )
        .eq("owner_id", ownerId)
  ...
  async getWithDays(
    ...
    calendar: Calendar & { orders?: any[] };
    days: CalendarDay[];
  } | null> {
    const { data: calendar, error: calError } = await (
    ...
    const { data: days, error: daysError } = await (
      supabase.from("calendar_days") as any
    )
      .select("*")
      .eq("calendar_id", id)
  ...
  async getPublic(
    ...
    const { data: calendars, error: calError } = await (
      supabase.from("calendars") as any
    )
    ...
    if (rpcError) {
      console.warn(
        "[CalendarsRepository] Error fetching owner premium status:",
        rpcError,
      );
    }
  ...
  async incrementViews(calendarId: string): Promise<void> {
    await (supabase.rpc as any)("increment_calendar_views", {
  ...
  async create(input: {
    ...
    const { data: themeDefaults } = await (
      supabase.from("theme_defaults") as any
    )
      .select("*")
    ...
    const { data: calendar, error: calError } = await (
      ...
      .insert({
        ...
        title:
          input.title ||
          (themeDefaults as any)?.default_title ||
          "Cápsula do Tempo",
    ...
    if (calError || !calendar)
      throw calError || new Error("Falha ao criar calendário");
  ...
  async updateDay(
    ...
    const { data, error } = await (supabase.from("calendar_days") as any)
      .update({
  ...
  async incrementShares(calendarId: string): Promise<void> {
    await (supabase.rpc as any)("increment_calendar_shares", {
      _calendar_id: calendarId,
    });
  },
  ...
  async uploadMedia(file: File, path: string): Promise<string> {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("day-media").getPublicUrl(data.path);

    return publicUrl;
  },

  async deleteMedia(path: string): Promise<void> {
    const { error } = await supabase.storage.from("day-media").remove([path]);
